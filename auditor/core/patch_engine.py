...
class PatchEngine:
    ...
    def generate_remediation_patch(
        ...
        target_file = self.root_dir / "sample_app" / "auth.py"
    ...
    def _fallback_rule_based_fix(self, current_code: str) -> str:
        """Deterministic fix for sample_app/auth.py without calling external APIs."""
        return '''import os
...
'''
    ...
    def apply_fix(self, new_content: str, target_relative_path: str = "sample_app/auth.py") -> bool:
        """Overwrites target file with generated fix."""
        target_file = self.root_dir / target_relative_path
        if target_file.exists() and new_content.strip():
